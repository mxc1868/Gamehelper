    ...
    public sealed partial class ShowMeWispCore
    {
        ...
        private void RecordChanges(WispObservation[] updated)
        {
            ...
            var changed = updated.Where(x => before.TryGetValue(x.Id, out var old) && old.Address == x.Address && old.Metadata == x.Metadata &&
                (old.Classification != x.Classification || old.Consumed != x.Consumed || old.StateKnown != x.StateKnown)).ToArray();
        ...
        private object RuntimeContext()
        {
            ...
            return new
            {
                Host = AssemblyInfo(typeof(Core).Assembly), Plugin = AssemblyInfo(typeof(ShowMeWispCore).Assembly),
