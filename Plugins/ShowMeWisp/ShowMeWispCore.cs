namespace ShowMeWisp
{
    ...
    public sealed partial class ShowMeWispCore : PCore<ShowMeWispSettings>
    {
        ...
        public override void OnEnable(bool isGameOpened)
        {
            ...
            try
            {
                var settingsToRead = WispSettingsFile.FindForRead(this.DllDirectory);
                if (File.Exists(settingsToRead))
                    this.Settings = JsonConvert.DeserializeObject<ShowMeWispSettings>(File.ReadAllText(settingsToRead)) ?? new();
            }
            catch (Exception ex)
            {
                this.Settings = new();
            ...
            WispApiEntity? evidence = null, bool recreateComponents = false)
        {
            ...
            var classification = WispClassifier.Classify(entity.Path, animatedPath, modelPath);
            if (evidence != null)
            {
                evidence.AnimatedPath = animatedPath;
                evidence.ModelPath = modelPath;
                evidence.Kind = classification?.Kind;
                evidence.Size = classification?.Size;
            }
            ...
            if (classification is { } resourceClass && WispClassifier.IsResource(resourceClass.Kind))
                Record(resourceClass.Size == WispSize.Unknown ? "wisp_size_unknown" : "wisp_size_known", resourceClass.Size.ToString());
