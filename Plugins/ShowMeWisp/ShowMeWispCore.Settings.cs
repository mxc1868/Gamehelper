namespace ShowMeWisp
{
    ...
    {
        private string Label(string key, string fallback) => this.PluginText.Label(key, fallback, "ShowMeWisp." + key);
        ...
        public override void DrawSettings()
        {
            ...
            ImGui.Checkbox(this.Label("settings.wisp_sizes", "Scale boxes by wisp size"), ref this.Settings.ScaleWispBoxes);
            ImGui.SliderFloat(this.Label("settings.small_wisp_scale", "Small wisp box scale"), ref this.Settings.SmallWispScale, 0.1f, 4);
            ImGui.SliderFloat(this.Label("settings.medium_wisp_scale", "Medium wisp box scale"), ref this.Settings.MediumWispScale, 0.1f, 4);
            ImGui.SliderFloat(this.Label("settings.big_wisp_scale", "Big wisp box scale"), ref this.Settings.BigWispScale, 0.1f, 4);
            ImGui.TextWrapped(this.PluginText.T("settings.wisp_sizes_note", "Applies to map and ground boxes. Unknown sizes keep the base size."));
            ...
            var resources = this.observations.Where(x => WispClassifier.IsResource(x.Classification.Kind)).ToArray();
            ImGui.Text(this.PluginText.F("diagnostics.sizes", "Wisp sizes: Small {0}; Medium {1}; Big {2}; unknown {3}",
                resources.Count(x => x.Classification.Size == WispSize.Small), resources.Count(x => x.Classification.Size == WispSize.Medium),
                resources.Count(x => x.Classification.Size == WispSize.Big), resources.Count(x => x.Classification.Size == WispSize.Unknown)));
