namespace WhereTheWispsAt
{
    ...
    {
        public static void Draw(IReadOnlyList<WispObservation> observations, WhereTheWispsAtSettings settings,
            Func<string, string, string> text, WispRenderReport? report)
        {
            ...
            try
            {
                foreach (var item in visible)
                {
                    ...
                    var size = isChest ? item.Bounds : new Vector3(settings.GroundWidth, settings.GroundWidth, settings.GroundHeight);
        ...
        private static void DrawMap(ImDrawListPtr draw, WispObservation[] items, WhereTheWispsAtSettings settings,
            ...
            float mapHeight, float zoom, Vector2 clipMin, Vector2 clipMax)
        {
            ...
            try
            {
                ...
                foreach (var item in items)
                {
                    ...
                    report?.Stages.Record(layer + "_marker_submitted", item.Id, item.Address, item.Metadata, position.ToString());
                    var color = ImGuiHelper.Color(GetColor(item.Classification.Kind, settings));
                    var half = new Vector2(settings.MarkerSize / 2);
                    ...
                    var labelPos = position + new Vector2(settings.MarkerSize, -labelSize.Y / 2);
        ...
        private static Vector4 GetColor(WispKind kind, WhereTheWispsAtSettings settings) => kind switch
        {
