...
- Current repository is local: `/home/ubuntu/Gamehelper`, branch `main`. The user deleted the `mxc1868/Gamehelper` GitHub fork; `origin` is a stale historical URL, not an active publishing destination. Do not recreate a fork or redirect pushes without a new instruction.
...
- Windows users have no compiler. Build on Linux with `scripts/package-wisps.py` (`--include-unique` for UniqueLoot); provide the full local self-contained x64 ZIP and checksum. GitHub Releases require a new user-specified destination.
